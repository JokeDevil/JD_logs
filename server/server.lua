...
exports('discord', function(message, id, id2, color, channel)
	...
	if Config.playerIP then _playerIP ="\n**Player IP:** " ..ids.ip.."" else _playerIP = "" end
	local player1 = message..'\n'.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL..''.._playerIP
	_message = player1
	...
	if Config.playerIP then _playerIP2 ="\n**Player IP:** " ..ids2.ip.."" else _playerIP2 = "" end
	local player2 = _playerID2..''.. _postal2 ..''.. _discordID2..''.._steamID2..''.._steamURL2..''.._playerIP2
	_message = player1..'\n'..player2
	end
...
AddEventHandler("playerConnecting", function(name, setReason, deferrals)
	...
	if Config.playerIP then _playerIP ="\n**Player IP:** " ..ids.ip.."" else _playerIP = "" end
	discordLog('**' .. sanitize(GetPlayerName(source)) .. '** is connecting to the server.'.. _discordID..''.._steamID..''.._steamURL..''.._playerIP..'', Config.joinColor, 'joins')
end)
...
AddEventHandler('playerDropped', function(reason)
	...
	if Config.playerIP then _playerIP ="\n**Player IP:** " ..ids.ip.."" else _playerIP = "" end
	discordLog('**' .. sanitize(GetPlayerName(source)) .. '** has left the server. (Reason: ' .. reason .. ')'.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL..''.._playerIP..'', Config.leaveColor, 'leaving')
end)
...
AddEventHandler('chatMessage', function(source, name, msg)
	...
	if Config.playerID then _playerID ="\n**Player ID:** " ..source.."" else _playerID = "" end
	if Config.playerIP then _playerIP ="\n**Player IP:** " ..ids.ip.."" else _playerIP = "" end
	discordLog('**' .. sanitize(GetPlayerName(source)) .. '**: ``' .. msg .. '``'.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL..''.._playerIP..'', Config.chatColor, 'chat')
end)
...
AddEventHandler('playerDied',function(id,player,killer,DeathReason,Weapon)
	...
	if id == 1 then  -- Suicide/died
        discordLog('**' .. sanitize(GetPlayerName(source)) .. '** '.._DeathReason..''.._Weapon..''.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL..''.._playerIP..'', Config.deathColor, 'deaths') -- sending to deaths channel
	...
	if Config.playerID then _killPlayerID ="\n**Player ID:** " ..killer.."" else _killPlayerID = "" end
		discordLog('**' .. GetPlayerName(killer) .. '** '.._DeathReason..' ' .. GetPlayerName(source).. ' `('.._Weapon..')`\n\n**[Player INFO]**'.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL.._playerIP..'\n\n**[Killer INFO]**'.._killPlayerID..''.. _postal2 ..''.. _KillDiscordID..''.._KillSteamID..''.._KillSteamURL..''.._playerIP2..'', Config.deathColor, 'deaths') -- sending to deaths channel
	else -- When gets killed by something else
        discordLog('**' .. sanitize(GetPlayerName(source)) .. '** `died`'.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL..''.._playerIP..'', Config.deathColor, 'deaths') -- sending to deaths channel
	end
end)
...
AddEventHandler('playerShotWeapon', function(weapon)
	...
	if Config.weaponLog then
		discordLog('**' .. sanitize(GetPlayerName(source))  .. '** fired a `' .. weapon .. '`'.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL..''.._playerIP..'', Config.shootingColor, 'shooting')
    end
end)
